...
describe("DaoVotingEscrow deployment", () => {
  ...
  it("Can deploy DaoVotingEscrow", async () => {
    ...
    expect(await veCrv.symbol()).to.equal("blCXD");
    expect(await veCrv.name()).to.equal("Boost-Lock CXD");
...
describe("Contract: DaoVotingEscrow", () => {
  ...
  describe("Defaults", () => {
    it("Symbol", async () => {
      expect(await blCxd.symbol()).to.equal("blCXD");
    });

    it("Name", async () => {
      expect(await blCxd.name()).to.equal("Boost-Locked CXD");
    });
