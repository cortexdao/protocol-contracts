...
const {ethers} = hre;
const {tokenAmountToBigNumber, impersonateAccount, getStablecoinAddress, forciblySendEth} = require("../utils/helpers")

async function main() {
  const [deployer] = await ethers.getSigners()

  const lpSafe = await impersonateAccount("0x5b79121EA6dC2395B8046eCDCE14D66c2bF221B0")
  const lpAccount = await ethers.getContractAt("LpAccount", "0x026544DACFAfC3422A0219cb6Be03ecB5D99a771", lpSafe)
  ...
  console.log("New DAI balance: %s", await daiToken.balanceOf(lpAccount.address))
}
